...
`ieee_mi.procedure_grid` is the cache/protocol-bound outer adapter for the four
...
1. Start from the unaltered broadband trial in `ieee-mi-cache-v2`.
...
tested `deepnet.local_outer_refit_benchmark` primitives. Therefore CAMEO,
...
- the analyzer decision code itself (`ieee_mi/procedure_grid_analysis.py`);
...
- Python executable, prefix, platform, UV version, PyTorch/CUDA runtime;
...
scikit-learn, MNE, pyRiemann, and PyTorch; the non-default `test` group must
...
```bash
uv run --frozen --no-sync --no-default-groups python -m ieee_mi.procedure_grid plan \
...
`.ieee-mi-project-gpu-leases`, shared by the procedure, common-recipe, and
...
CUDA_VISIBLE_DEVICES=GPU-... uv run --frozen --no-sync --no-default-groups \
  python -m ieee_mi.procedure_grid worker \
  --run-root /path/to/runs/binary_procedure_grid_v1 \
  --cache-root /path/to/data_cache \
...
`.ieee-mi-project-gpu-lease-forensics` tree and atomically renamed into the
...
```bash
uv run --frozen --no-sync --no-default-groups python -m ieee_mi.procedure_grid audit \
  --run-root /path/to/runs/binary_procedure_grid_v1
```
...
```bash
uv run --frozen --no-sync --no-default-groups python -m ieee_mi.procedure_grid_analysis \
  --run-root /path/to/runs/binary_procedure_grid_v1 \
  --cache-root /path/to/data_cache \
...
uv run --frozen --no-sync --no-default-groups --group test python -m pytest -q \
  ieee_mi/tests/test_project_gpu_leases.py \
  ieee_mi/tests/test_procedure_grid.py \
  ieee_mi/tests/test_procedure_grid_analysis.py
```
