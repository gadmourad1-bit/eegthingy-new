...
The cache contract is `ieee-mi-cache-v2/harmonized`: 128 Hz, the half-open
...
`ieee_mi.hemiq_v2_grid build-plan` creates the run directories and publishes a
...
Every worker acquires `ieee_mi.project_gpu_leases` with track scope
...
`ieee_mi.hemiq_v2_analysis` has no training entry point. It first acquires an
...
```bash
uv run --active --offline --no-sync python -m ieee_mi.hemiq_v2_grid \
  build-plan \
...
```bash
uv run --active --offline --no-sync python -m ieee_mi.hemiq_v2_analysis \
  --project-root /home/hanafy/neuralnetwork \
  --cache-root /path/to/cache \
  --run-root /path/to/hemiq-v2-run \
