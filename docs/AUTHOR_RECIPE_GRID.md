...
`ieee_mi.author_recipe_grid` is the executable adapter for the two
...
- the sole audited executor
  `ieee_mi.author_recipe_grid:execute_author_recipe_job` (there is no formal
...
- `IEEE_MI_TCFORMER_ROOT` pointing to the reviewed compact TCFormer runtime
...
export IEEE_MI_TCFORMER_ROOT=/home/hanafy/neuralnetwork/third_party/TCFormer
```
...
```bash
uv run --frozen --no-default-groups --no-sync \
  python -m ieee_mi.author_recipe_grid run \
...
```bash
uv run python -m ieee_mi.author_recipe_grid status \
  --run-root /home/hanafy/scratchpad/author_recipe_grid_v1

uv run python -m ieee_mi.author_recipe_grid audit \
  --run-root /home/hanafy/scratchpad/author_recipe_grid_v1
```
...
```bash
uv run python -m ieee_mi.author_recipe_analysis \
  --run-root /home/hanafy/scratchpad/author_recipe_grid_v1 \
  --cache-root /home/hanafy/scratchpad/eeg_novel_20260729/data_cache \
