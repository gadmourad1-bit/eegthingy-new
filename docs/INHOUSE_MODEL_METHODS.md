...
[`ieee_mi/model_registry.py`](../ieee_mi/model_registry.py). Architecture
...
The RA-L draft at [`paper/main.tex`](../paper/main.tex) is a separate
...
[`deepnet_CHATGPT_HANDOFF.md`](../deepnet_CHATGPT_HANDOFF.md) explicitly says
...
- **C** — Five-seed local Exp4 common-recipe development tournament:
  [`final_ranking.json`](../ieee_mi/results/local_exp4_allmodels_20260721/neural/final_ranking.json)
  and
  [`LOCAL_ALL_50_FINAL.md`](../ieee_mi/results/local_exp4_allmodels_20260721/LOCAL_ALL_50_FINAL.md).
  Recording 4 had informed development, so this is post-selection development,
  not independent confirmation.

- **S** — Single-seed four-dataset Cardinal screens summarized in
  [`MODEL_ACCURACY_INVENTORY.md`](../MODEL_ACCURACY_INVENTORY.md) and
  [`ieee_mi/EXPERIMENT_LEDGER.md`](../ieee_mi/EXPERIMENT_LEDGER.md). No v4-v6
  candidate passed the prespecified architecture-advancement gate.

- **G** — GeoAdaptNet-family experiments in
  [`deepnet/RESULTS.md`](../deepnet/RESULTS.md) and the JSONs under
  `deepnet/results/`. These mix deployment, architecture-only, and diagnostic
  protocols and must not be pooled into one score.

- **L** — Exact current local outer-refit procedure artifact under
  [`ieee_mi/results/local_exp4_allmodels_20260721/procedures`](../ieee_mi/results/local_exp4_allmodels_20260721/procedures),
  with the same outer rows as the local tournament but a separate
  procedure-specific covariance/reflection path.

- **D** — Development artifacts only, usually under `deepnet/results/cameo`
  or `deepnet/results/parity`. The cohort/protocol named in the artifact
...
- **Q** — HemiQ's one-time BNCI2014-004 confirmation, frozen manifest, and
  ...
  [`deepnet/HEMIQ_FIELD_REPORT.md`](../deepnet/HEMIQ_FIELD_REPORT.md). This is
  five healthy confirmation subjects, not a population-superiority or
  clinical result. The frozen
  [`manifest`](../deepnet/results/hemi_q/hemi_q_final_frozen_manifest.json)
  hashes to
  `2bbe28d4415a74cb007328579cf8153585ceaaa28219246aecd0622398987401`;
  the
  [`confirmation`](../deepnet/results/hemi_q/hemi_q_final_bnci004_confirmation_s5_9.json)
  and permanent
  [`receipt`](../deepnet/results/hemi_q/.confirmation-receipts/eegthingy-hemiq-bnci2014-004-s5-9-final-v1.json)
  hashes are recorded in the report.

- **FBC-T** — CardinalFBC native-transfer fold-0/seed-7 development screen in
  [`NATIVE_TRANSFER_FOLD0_SEED7_SUMMARY.md`](../ieee_mi/results/NATIVE_TRANSFER_FOLD0_SEED7_SUMMARY.md).
  The candidate failed its paper-lead gate.

- **FBMS-T** — Complete five-seed/all-fold CardinalFBMS development grid in
  [`NATIVE_FBMS_FULL_GRID_SUMMARY.md`](../ieee_mi/results/NATIVE_FBMS_FULL_GRID_SUMMARY.md).
  The frozen development gate passed; this is still opened development
  evidence, not sealed confirmation or SOTA evidence. The
  [`full-grid gate`](../ieee_mi/results/native_fbms_transfer_full_grid_gate.json)
  hashes to
...
`ieee_mi.baselines.make_model(<common_roster_name>, ...)`, implemented in
[`ieee_mi/baselines.py`](../ieee_mi/baselines.py), and trained as raw-trial
architectures under the common track. The classes themselves are in
[`ieee_mi/models.py`](../ieee_mi/models.py). They accept variable output counts
...
[`ieee_mi/CARDINAL_FIELD_THEORY.md`](../ieee_mi/CARDINAL_FIELD_THEORY.md).
...
`deepnet.model.GeoAdaptNet` consumes four fixed-band SPD covariance matrices.
...
`deepnet.filterbank_net.FilterBankSPDNet` instead learns a Sinc bandpass bank
...
`deepnet.cameo_net.CAMEOClassifier` combines a frozen source-only convex
...
artifacts
([local](../deepnet/results/cameo/cameo_v1_frozen_local_confirm_3seeds.json),
[Cho](../deepnet/results/cameo/cameo_v1_frozen_cho_confirm27_52_f5.json)), as
well as the exact current local
[outer-refit artifact](../ieee_mi/results/local_exp4_allmodels_20260721/procedures/cameo.json).
...
`deepnet.parity_net.HemiParityClassifier` forms internal even and odd raw and
...
`deepnet.parity_fuse_net.ParityFuseClassifier` tightens the same group action:
...
exist, plus current local outer-refit artifacts for
[HemiParity](../ieee_mi/results/local_exp4_allmodels_20260721/procedures/hemiparity.json)
and
[PARITY-Fuse](../ieee_mi/results/local_exp4_allmodels_20260721/procedures/parity_fuse.json).
...
`deepnet.hemi_q_field_net.HemiQFieldClassifier` is specific to the three
...
`ieee_mi.dual_view.CardinalSplineDualViewNet` evaluates one shared CardinalFBC
...
`ieee_mi.dual_view_training.fit_dual_view_target`.
...
`deepnet.orbit_transport_net.OrbitTransportClassifier` takes arbitrary expert
...
[confirmation](../deepnet/results/parity/orbit_transport_final_bnci_confirmation_s5_9.json),
frozen OrbitTransportNet scored below the tangent and Riemannian controls. The
current local
[ORBIT-v3 artifact](../ieee_mi/results/local_exp4_allmodels_20260721/procedures/orbit_v3.json)
...
[`ieee_mi/native_transfer.py`](../ieee_mi/native_transfer.py), with the atomic
call `run_record(..., condition=<key>)`. The CardinalFBMS implementation is
[`ieee_mi/native_fbms_transfer.py`](../ieee_mi/native_fbms_transfer.py), also
through `run_record`.
...
|---|---|---|---|---|
| `common.scope` — SCOPE-Net | distinct in-house architecture | `ieee_mi.models.ScopeNet`; `make_model("scope")` | Coordinate-continuous Sinc/scalp-field energy-dynamics model | **C**; weak local result, no confirmation |
| `common.free_scope` — FreeSCOPE | SCOPE configuration/control | `ieee_mi.models.FreeScopeNet`; `make_model("free_scope")` | Channel-indexed control for coordinate continuity | **C**; control exceeded SCOPE locally |
| `common.cardinal` — CardinalField | distinct in-house architecture | `ieee_mi.models.CardinalFieldNet`; `make_model("cardinal")` | 21-anchor cardinal-RBF field plus ordered Sinc/log variance | **C**; did not lead locally |
| `common.free_cardinal` — FreeCardinal | CardinalField configuration/control | `ieee_mi.models.FreeCardinalFieldNet`; `make_model("free_cardinal")` | Channel-indexed CardinalField control | **C**; exceeded coordinate model locally |
| `common.cardinal_dynamics` — CardinalDynamics | distinct in-house architecture | `ieee_mi.models.CardinalDynamicsNet`; `make_model("cardinal_dynamics")` | Default full-rank continuous energy+dynamics field | **C**; 21-anchor configuration only |
...
| `common.cardinal_fbc` — CardinalFBC | distinct in-house **FBCNet derivative** | `ieee_mi.models.CardinalFBCNet`; `make_model("cardinal_fbc")` | Replace indexed FBCNet spatial convolution with 21-anchor field | **C+S**; public FBCNet must be cited; no gate pass |
...
| `common.cardinal_fbc_corr` — CardinalFBC correlation | family configuration, FBCNet derivative | `ieee_mi.models.CardinalFBCCorrelationNet`; `make_model("cardinal_fbc_corr")` | Zero-started low-rank Fisher-z correlation continuation | **C+S**; dataset-inconsistent screen gains |
...
| `common.cardinal_fbc_compactdyn` — CardinalFBC compact dynamics | family configuration, FBCNet derivative | `ieee_mi.models.CardinalFBCCompactDynamicsNet`; `make_model("cardinal_fbc_compactdyn")` | Zero-started compact energy/dynamics continuation | **C+S**; no advancement |
...
| `common.cardinal_fbc_micro` — CardinalFBC micro dynamics | family configuration, FBCNet derivative | `ieee_mi.models.CardinalFBCMicroDynamicsNet`; `make_model("cardinal_fbc_micro")` | Zero-started learned-Gabor micro continuation | **C+S**; no gate pass |
...
| `common.cardinal_fbc_physical` — CardinalFBC ordered physical dynamics | family configuration, FBCNet derivative | `ieee_mi.models.CardinalFBCPhysicalDynamicsNet`; `make_model("cardinal_fbc_physical")` | Zero-started ordered-Sinc energy/dynamics continuation | **C+S**; no gate pass |
...
| `common.cardinal_fbms` — CardinalFBMS | distinct in-house **FBMSNet derivative** | `ieee_mi.models.CardinalFBMSNet`; `make_model("cardinal_fbms")` | Replace indexed FBMSNet spatial convolution with 21-anchor field | **C+S**; common model failed v6 gate |
...
| `common.cardinal_mix` — CardinalMixedTemporal | distinct in-house **FBMSNet-component derivative** | `ieee_mi.models.CardinalMixedTemporalNet`; `make_model("cardinal_mix")` | FBMS temporal components + full-rank cardinal field + new decoder | **C**; weak/high-variance local result |
...
|---|---|---|---|---|
| `architecture.geoadaptnet` — GeoAdaptNet | distinct architecture within a deployment procedure | `deepnet.model.GeoAdaptNet`; `deepnet.engine.train_model` | L/B/C/P binary; tangent anchor plus gated SPD residual and optional intent head | **G**; residual inert, classical/deep baselines often stronger |
| `architecture.geoadaptnet_fb` — GeoAdaptNet-FB | distinct learnable-filter geometric architecture | `deepnet.filterbank_net.FilterBankSPDNet`; `FilterBankSPDClassifier(reduced_dim=None).fit(...)` | L/B/C/P binary; learn Sinc bands before full covariance/log tangent | **G**; modest Cho gain, still below ShallowConvNet |
...
| `architecture.cameo` — CAMEO-Net | distinct architecture plus validation routing | `deepnet.cameo_net.CAMEOClassifier`; local `deepnet.local_outer_refit_benchmark.run_one(model_name="cameo", ...)` | L/B/C/P binary; counterfactual tangent/energy/dynamics experts | **D+L**; procedure-specific, protocols cannot be pooled |
| `architecture.hemiparity` — HemiParityNet | distinct binary equivariant architecture | `deepnet.parity_net.HemiParityClassifier`; local `run_one(model_name="hemiparity", ...)` | L/B/C/P binary; internal even/odd raw+tangent fusion | **D+L**; legacy A is binary and incompatible with current A |
| `architecture.parity_fuse` — PARITY-Fuse | distinct binary equivariant architecture | `deepnet.parity_fuse_net.ParityFuseClassifier`; local `run_one(model_name="parity_fuse", ...)` | L/B/C/P binary; deterministic latent parity fusion | **D+L**; did not lead local table |
| `architecture.hemi_q_field` — HemiQ-FieldNet | distinct dataset-specific architecture | `deepnet.hemi_q_field_net.HemiQFieldClassifier`; `deepnet.hemi_q_benchmark.run(...)` | B only; exact odd cross-spectral field on supplied bipolar signals | **Q**; five-subject confirmation, tied strong public baseline |
| `architecture.cardinal_spline_dual_view` — CardinalSplineDualView | distinct bounded transfer prototype | `ieee_mi.dual_view.CardinalSplineDualViewNet`; `ieee_mi.dual_view_training.fit_dual_view_target` | C/P target cohorts; shared native/canonical view fusion | **P**; no aggregate result or production writer |
| `architecture.orbit_v1` — ORBIT-v1 | distinct OrbitTransportNet architecture | `deepnet.orbit_transport_net.OrbitTransportClassifier`; v1 artifact config | L/B/C/P binary; exact odd transport of arbitrary expert pairs | **D**; no current all-dataset adapter |
...
|---|---|---|---|---|
| `procedure.pretrained_cardinal_fbc` — Pretrained CardinalFBC | procedure configuration | `ieee_mi.native_transfer.run_record(condition="pretrained_cardinal_fbc")` | Source-pretrained coordinate field fine-tuned on native C/P target | **FBC-T**; fold-0/seed-7 only, failed lead gate |
| `procedure.scratch_cardinal_fbc` — Scratch CardinalFBC | procedure configuration | `ieee_mi.native_transfer.run_record(condition="scratch_cardinal_fbc")` | Matched from-scratch CardinalFBC target control | **FBC-T**; not an independent architecture |
| `procedure.pretrained_cardinal_fbms` — Pretrained CardinalFBMS | procedure configuration | `ieee_mi.native_fbms_transfer.run_record(condition="pretrained_cardinal_fbms")` | Byte-pinned source checkpoint fine-tuned on native C/P target | **FBMS-T**; development gate pass, no sealed confirmation |
| `procedure.scratch_cardinal_fbms_canonical_seeded` — Scratch CardinalFBMS, canonical seeded | procedure configuration | `ieee_mi.native_fbms_transfer.run_record(condition="scratch_cardinal_fbms_canonical_seeded")` | Scratch target from canonical field parameterization, no source weights | **FBMS-T**; scratch control, not a new family |
| `procedure.scratch_cardinal_fbms_native_projected` — Scratch CardinalFBMS, native projected | procedure configuration | `ieee_mi.native_fbms_transfer.run_record(condition="scratch_cardinal_fbms_native_projected")` | Project a fresh native indexed layer into the field | **FBMS-T**; scratch initialization ablation |
...
|---|---|---|---|
| `chsdnet` | `ieee_mi.chsd.CHSDNet`; `make_chsd_model` | Native-montage multitaper complex coherency HPD surface; Hermitian matrix-log state plus explicit time, frequency, and mixed finite differences; factorized decoder plus separate power/ERD branch | Implementation and tests; no result artifact stored in this repository |
| `chsdnet_direct` | `ieee_mi.chsd_direct.CHSDDirectNet`; `make_chsd_direct_model` | Flattened native HSD fields with an active state head and zero-started differential/power heads | Implementation and tests; unregistered development ablation |
| `chsdnet_hybrid` | `ieee_mi.chsd_hybrid.CHSDResidualNet`; `make_chsd_hybrid_model` | CardinalFBC-micro-extended logits plus a bounded zero-started CHSD residual logit | Implementation and tests; unregistered development ablation |
| `chsdnet_joint` | `ieee_mi.chsd_joint.CHSDJointNet`; `make_chsd_joint_model` | Append CHSD summaries as zero columns in CardinalFBC's one constrained head | Implementation and tests; unregistered development ablation |
| `chsdnet_conditioned_005` | `ieee_mi.chsd_conditioned.CHSDSurfaceConditionedCardinalFBC`; `make_chsd_conditioned_model` | HSD-conditioned multiplicative modulation of FBC band-by-view floor, bounded at 0.05 | Completed 115-participant, five-dataset disjoint-subject robustness amendment; decision `stop_chsd_promotion` |
...
4. Every common family configuration shares a class with one or more siblings.
   `common_roster_name` plus `ieee_mi.baselines.make_model` is therefore part
