...
project that will be promoted to `/home/hanafy/neuralnetwork`. It is a design
and audit record only. The repository's current `pyproject.toml` and `uv.lock`
...
The most important current-state warning is:

> The repository-root `uv.lock` is not the benchmark lock. Do not run
> `uv sync`, `uv sync --frozen`, or an automatic `uv run` sync against the
...
audit showed that the current root project would remove the research packages
...
final project gets a new `.venv` built from a new, purpose-specific lock.

## Audit snapshot: 2026-07-29

### Repository root

The current root project describes the broader Mac acquisition, GUI, and
robotics application, not the paper benchmark.
...
be changed by this project. The cu124 Torch wheel supplies its user-space CUDA
runtime. No global CUDA or cuDNN installation is required.
...
NumPy, SciPy, scikit-learn, Torch, MNE, pyRiemann, and Braindecode. MOABB is
imported lazily by the dataset loaders. The pinned official TCFormer source
imports `einops`. The CPU control runner imports `threadpoolctl` at execution
time and explicitly requires it to be a direct project dependency.
...
| `full_grid` and `full_grid_analysis` | GPU producer plus CPU analysis | Freeze all installed distributions, Python/UV/driver/Torch identity, source closure, extra factory, executor, `pyproject.toml`, and `uv.lock` |
...
The exact Torch direct requirement includes the wheel's PEP 440 local build
tag, `2.6.0+cu124`, and the explicit cu124 index must resolve and install that
same version string. This is stricter than PyTorch's generic installation
command because the formal validators compare the declared, locked, and
installed versions exactly. It follows the official
...
`torchaudio` and `torchvision` are intentionally absent. No benchmark source
imports them, and Braindecode documents that torchvision is not required for
this use. Adding unused binary packages increases the solver surface and disk
use. `rotary-embedding-torch`, skorch, joblib, pooch, tqdm, h5py, MNE-BIDS, and
the NVIDIA user-space wheels may remain transitive dependencies, but their
exact versions and distribution hashes must be present in `uv.lock`.
...
3.12.13 resolved all 109 packages, including Torch 2.6.0+cu124, pytest 9.0.2,
and ReportLab 5.0.0. The probe `pyproject.toml` SHA-256 was
...
This proves resolution only; it is not the final project lock or a substitute
for the clean online/offline environment proofs.
...
- Braindecode 1.6.1 requires Python at least 3.11 and works in the observed
  Python 3.12.13/Torch 2.6.0 environment.
...
4. A driver update, Python patch update, UV minor update, package addition, or
...
```bash
uv tree --frozen
rg 'name = "(torch|braindecode|moabb|mne|numpy|pandas|pyriemann|scikit-learn|scipy|threadpoolctl)"' uv.lock
sha256sum pyproject.toml .python-version uv.lock
```

The lock must identify Torch's `pytorch-cu124` registry and must include the
Linux x86-64 cu124 wheel. `uv lock --check` is important because `--frozen`
uses the lock as the source of truth without checking whether project metadata
is newer. The required pair for formal use is therefore:
...
uv run --frozen --no-default-groups python -c \
  'import torch; print(torch.__version__); print(torch.version.cuda); print(torch.backends.cudnn.version()); print(torch.cuda.get_device_name(0)); print(torch.cuda.get_device_capability(0))'
```

Required values are Torch `2.6.0+cu124`, CUDA build `12.4`, cuDNN `90100`, and
exactly one visible approved GPU. A different driver version, build tag, cuDNN
value, or device mapping is a failed preflight, not a reason to install or
upgrade global software.
...
uv run --frozen --no-default-groups python -c \
  'import braindecode,mne,moabb,numpy,pandas,pyriemann,scipy,sklearn,torch; print("imports-ok")'
```
...
uv run --frozen --no-default-groups --group test \
  python -m pytest -q ieee_mi/tests deepnet/tests
```
...
CUBLAS_WORKSPACE_CONFIG=:4096:8 \
UV_PROJECT_ENVIRONMENT=/home/hanafy/neuralnetwork/.venv-test \
uv run --frozen --no-default-groups --group test python -m pytest -q \
...
7. Torch version, Torch CUDA build, cuDNN integer, CUDA availability, visible
   device count, GPU name, full physical UUID, and compute capability;
