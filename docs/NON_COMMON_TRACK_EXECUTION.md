...
`ieee_mi.track_matrix`. The module produces an exact 70-model × 5-dataset
...
```bash
uv run python -m ieee_mi.track_matrix
```
...
```bash
uv run python -m ieee_mi.track_matrix --json
```

No command in `ieee_mi.track_matrix` loads EEG, builds a cache, imports a
...
| `separate_eligible` | 37 | Registry-eligible and backed by a cache/protocol-bound executor, but report in its author-faithful/procedure/control track. |
...
| `missing_adapter_or_blocker` | 51 | Scientifically eligible in the registry, but no current executor binds the implementation to the locked cache, split, reset/refit, and artifact contracts. |
...
`ieee_mi/results/local_exp4_allmodels_20260721`, but their five repeated seed
...
- `architecture.geoadaptnet` constructs `deepnet.model.GeoAdaptNet` and is
  optimized by `deepnet.engine.train_model`. It consumes four deterministic
  bandwise SPD covariances and has a hard binary main head.
- `architecture.geoadaptnet_fb` and
  `architecture.geoadaptnet_fbsp` use
  `deepnet.filterbank_net.FilterBankSPDClassifier.fit` on broadband trials.
...
- The exact local v2 path is
  `deepnet.local_outer_refit_benchmark.run_benchmark`. It owns cache loading,
...
`deepnet.hemi_q_field_net.HemiQFieldClassifier.fit` is valid only for the
...
- `reference.tcformer` calls
  `ieee_mi.reference_training.fit_tcformer_reference` with the pinned official
...
- `reference.fbcnet` calls
  `ieee_mi.reference_training.fit_fbcnet_reference`. Raw v2 trials first pass
...
- `ieee_mi.native_transfer.run_record` resolves four explicit runtime
  conditions. The stable ID
  `procedure.scratch_fbmsnet_legacy_transfer` maps to the shorter runtime key
  `scratch_fbmsnet`.
- `ieee_mi.native_fbms_transfer.run_record` resolves five CardinalFBMS-family
  conditions. The exact completed grid is 4,625 Cho records plus 4,050
  PhysioNet records.
- Every native-transfer row requires `ieee-mi-cache-v2/native`, a byte-pinned
...
- `CardinalSplineDualViewNet` and
  `ieee_mi.dual_view_training.fit_dual_view_target` implement a leakage-safe
...
- `deepnet.baselines.RiemannianTangentLogistic.fit`;
- `deepnet.tangent_anchor.TangentAnchorClassifier.fit`; and
- `deepnet.baselines.EAFilterBankCSP.fit`.

The cache/protocol-bound outer entrypoint is `ieee_mi.control_grid.main`. Its
...
`ieee_mi.full_grid.execute_benchmark_job`.
