...
def test_row_identity_preserves_established_digest_contract() -> None:
    ...
    expected = "935dafd549c76716ff5da7a95e98f853485b0ff71b0d5d7049851eb2950cc6f6"
    assert control_grid._rows_sha256(rows) == expected
    assert control_grid._partition_identity(rows) == {
        "count": 4,
...
def _tiny_plan(
    *,
    ...
    return control_grid.assemble_plan(
        ...
        source_identity={"src/benchmark/control_grid.py": "b" * 64},
...
def test_tangent_anchor_transitive_source_is_frozen() -> None:
    assert "src/benchmark/shared/spd.py" in control_grid.SOURCE_FILES
