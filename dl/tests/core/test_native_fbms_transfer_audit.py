...
from benchmark import native_fbms_pretraining_cli as pretrain
from benchmark import native_fbms_transfer as transfer
from benchmark import native_fbms_transfer_audit as audit
from benchmark import native_fbms_transfer_gate as gate
...
def _write_record(
    ...
    *,
    ...
    source_files = {
        "eeg_mi/native_fbms_pretraining_cli.py": hashlib.sha256(
            b"pretraining-source"
        ).hexdigest(),
        "eeg_mi/native_fbms_transfer.py": hashlib.sha256(
            b"transfer-source"
        ).hexdigest(),
        "eeg_mi/models.py": hashlib.sha256(b"model-source").hexdigest(),
    }
    ...
    if effective_model == "cardinal_fbms":
        model_class = "eeg_mi.models.CardinalFBMSNet"
        model_config = {
