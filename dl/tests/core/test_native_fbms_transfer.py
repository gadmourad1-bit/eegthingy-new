...
from benchmark import native_fbms_pretraining_cli as pretrain_cli
from benchmark import native_fbms_transfer as transfer
from benchmark import legacy_native_identity
from benchmark import native_pretraining_cli as pretrain_core
...
def test_historical_native_fbms_source_archive_is_exact_and_nonimportable() -> None:
    observed = legacy_native_identity.verify_archive()
    assert observed == legacy_native_identity.PINNED_SOURCE_MANIFEST
    encoded = json.dumps(
    ...
    assert not (legacy_native_identity.ARCHIVE_ROOT / "__init__.py").exists()
...
def _strict_checkpoint_payload(
    *, source_hashes: dict[str, str] | None = None
    ...
    if source_hashes is None:
        source_hashes = {
            name: digest
            for name, digest in legacy_native_identity.PINNED_SOURCE_MANIFEST.items()
            if name
            not in {
    ...
    return {
        ...
        "model": {
            ...
            "class": transfer._legacy_logical_class(type(model)),
            "construction": {
                "n_channels": 21,
                "n_outputs": 2,
                "n_times": 320,
        ...
        "state_hashes": {
            "initial": (
                legacy_native_identity.PINNED_LEGACY_SOURCE_INITIAL_STATE_SHA256
            ),
...
def test_transfer_surface_is_the_exact_independent_five_condition_family() -> None:
    ...
    assert transfer.ARTIFACT_SCHEMA.startswith(
        "eeg-mi-native-cardinal-fbms-transfer-"
    )
...
def test_family_and_condition_guards_fail_before_target_cache_io(
    ...
    monkeypatch.setattr(
        transfer,
        "_source_file_hashes",
        lambda: {"eeg_mi/native_fbms_transfer.py": "1" * 64},
    )
    monkeypatch.setattr(
        transfer,
    ...
    with pytest.raises(RuntimeError, match="writer is retired"):
        transfer.run_record(
            dataset="cho2017",
            subject=16,
            fold=0,
...
def test_strict_checkpoint_loader_pins_family_state_corpus_and_sources(
    ...
    source_hashes = dict(payload["source_code"]["files"])
    source_hashes["eeg_mi/models.py"] = "0" * 64
    stale_path = tmp_path / "stale-source.pt"
    torch.save(
        _strict_checkpoint_payload(source_hashes=source_hashes), stale_path
    )
    with pytest.raises(RuntimeError, match="historical source"):
        transfer.load_immutable_native_checkpoint(
            stale_path,
    ...
    with pytest.raises(RuntimeError, match="pinned legacy writer identity"):
        transfer.load_immutable_native_checkpoint(
            initial_path,
            expected_file_sha256=hashlib.sha256(
...
def test_historical_checkpoint_reader_does_not_regenerate_runtime_rng_state(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    checkpoint_path = tmp_path / "checkpoint.pt"
    torch.save(_strict_checkpoint_payload(), checkpoint_path)
    file_hash = hashlib.sha256(checkpoint_path.read_bytes()).hexdigest()

    def unexpected_runtime_regeneration(*args: object, **kwargs: object) -> object:
        del args, kwargs
        raise AssertionError("historical reader regenerated host-dependent RNG state")

    monkeypatch.setattr(transfer, "_scratch_state", unexpected_runtime_regeneration)
    loaded = transfer.load_immutable_native_checkpoint(
        checkpoint_path, expected_file_sha256=file_hash
    )
    assert loaded.pretraining["state_hashes"]["initial"] == (
        legacy_native_identity.PINNED_LEGACY_SOURCE_INITIAL_STATE_SHA256
    )
...
def test_record_publication_is_atomic_family_typed_and_write_once(
    ...
    monkeypatch.setattr(
        ...
        "_source_file_hashes",
        lambda: {
            "eeg_mi/native_fbms_pretraining_cli.py": "1" * 64,
            "eeg_mi/native_fbms_transfer.py": "2" * 64,
        },
    )
    monkeypatch.setattr(
        transfer._core,
    ...
    assert calls == {"checkpoint": 0, "target": 0, "evaluation": 0}
