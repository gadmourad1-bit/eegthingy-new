...
import benchmark.confirmation as confirmation_module
import benchmark.data as data_module
import benchmark.freeze as freeze_module
from benchmark.config import channels_for_dataset, dataset_spec
from benchmark.confirmation import (
    COMPLETED_RECEIPT,
...
from benchmark.freeze import create_freeze_manifest
...
def test_current_source_bytes_are_revalidated_before_callbacks(tmp_path: Path) -> None:
    project, manifest_path, run_root, _ = _frozen_run(tmp_path)
    initialize_confirmation_run(manifest_path, run_root, project_root=project)
    (project / "src" / "benchmark" / "runner.py").write_text(
