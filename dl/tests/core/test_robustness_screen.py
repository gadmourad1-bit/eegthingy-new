...
from benchmark.data import split_indices
    ...
    for dataset, subjects in ROBUSTNESS_COHORTS:
        for subject in subjects:
            ...
            cache_identity[subject_key] = {
                "relative_path": (
                    f"eeg-mi-cache-v2/harmonized/{dataset}/subject_{subject:03d}.npz"
                ),
                "file_size_bytes": 123,
...
def lineage() -> dict[str, object]:
    return {
        ...
        "conditioned_v3": {
            ...
            "plan_schema": ("eeg-mi-chsd-conditioned-development-screen-plan-v3"),
...
def test_manifest_freezes_lineage_decision_rule_sources_and_four_gpus() -> None:
    ...
    assert "src/benchmark/robustness_analysis.py" in plan["source_identity"]
    assert [
        binding["gpu_uuid"]
...
def test_manifest_rejects_history_rewrite_cache_drift_and_gpu_alias() -> None:
    ...
    bad_cache[first_key]["relative_path"] = (  # type: ignore[index]
        "eeg-mi-cache-v2/harmonized/wrong/subject_001.npz"
    )
