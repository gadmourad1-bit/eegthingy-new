...
PLAN_SCHEMA = "eeg-mi-score-blind-control-grid-plan-v1"
RECORD_SCHEMA = "eeg-mi-score-blind-control-record-v1"
COMPLETION_SCHEMA = "eeg-mi-score-blind-control-completion-v1"
CLAIM_SCHEMA = "eeg-mi-control-grid-claim-v1"
CPU_SLOT_SCHEMA = "eeg-mi-control-grid-cpu-slot-v1"
FAILURE_SCHEMA = "eeg-mi-control-grid-failure-v1"
AUDIT_SCHEMA = "eeg-mi-score-blind-control-audit-v1"
...
IMPLEMENTATIONS: Mapping[str, str] = {
    "control.riemann": "benchmark.research.baselines.RiemannianTangentLogistic",
    "control.tangent_anchor": "benchmark.research.tangent_anchor.TangentAnchorClassifier",
    "control.ea_fbcsp": "benchmark.research.baselines.EAFilterBankCSP",
}
...
SOURCE_FILES: tuple[str, ...] = (
    "src/benchmark/control_grid.py",
    "src/benchmark/config.py",
    "src/benchmark/data.py",
    "src/benchmark/local_geometric_controls.py",
...
_ROWS_HASH_COMPATIBILITY_PREFIX = bytes(
    (
        105, 101, 101, 101, 45, 109, 105, 45, 99, 111, 110, 116, 114, 111,
        108, 45, 114, 111, 119, 45, 105, 110, 100, 105, 99, 101, 115, 45,
        118, 49, 0,
    )
)
...
def _coordination_lock(
    ...
    *,
    ...
    payload = b"eeg-mi-control-grid-coordination-v1\n"
...
def _new_estimator(control: str, parameters: Mapping[str, Any]) -> Any:
    if control == "control.riemann":
        from benchmark.research.baselines import RiemannianTangentLogistic

        return RiemannianTangentLogistic(
            c=float(parameters["c"]), max_iter=3000
        )
    if control == "control.tangent_anchor":
        from benchmark.research.tangent_anchor import TangentAnchorClassifier

        return TangentAnchorClassifier(
            C=float(parameters["C"]), max_iter=3000
        )
    if control == "control.ea_fbcsp":
        from benchmark.research.baselines import EAFilterBankCSP
