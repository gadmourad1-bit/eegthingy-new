...
> **Retired execution identity.** The earlier external checkpoint and record
> metadata are read-only and are not accepted by the sanitized release. The
> workload and commands below document the frozen procedure; they are not a
> supported resume path. A future execution requires a newly authorized
> checkpoint, source manifest, cache identity, and result root.
...
## Archived GPU-box command shape

First synchronize and test the code. Then run the score-blind preflight (no
`--execute`):

```bash
cd /home/user/Desktop/eegthingy_eeg_arch_v7_20260719
...
PYTHONPATH=. .venv/bin/python -m benchmark.native_fbms_full_grid_ops \
