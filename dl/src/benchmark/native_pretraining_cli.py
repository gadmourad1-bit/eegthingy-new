...
def _source_paths(
    ...
    paths = {
        "src/benchmark/baselines.py": package / "baselines.py",
        "src/benchmark/config.py": package / "config.py",
        "src/benchmark/data.py": package / "data.py",
        "src/benchmark/models.py": package / "models.py",
        "src/benchmark/native_pretraining.py": package / "native_pretraining.py",
        "src/benchmark/native_pretraining_cli.py": Path(__file__).resolve(),
        "src/benchmark/training.py": package / "training.py",
        "src/benchmark/requirements-cu128.txt": package / "requirements-cu128.txt",
    }
...
def execute_locked_native_pretraining(
    ...
    *,
    ...
    _validate_entry_point(spec)
    if spec.checkpoint_schema == (
        "eeg-mi-native-cardinal-fbms-pretraining-checkpoint-v1"
    ):
        raise RuntimeError(
