...
This runner is deliberately separate from :mod:`benchmark.local_model_tournament`.
...
``eeg-mi-cache-v2`` broadband trials.
...
SCHEMA = "eeg-mi-local-geometric-controls-v1"
DATASET = "local_exp4"
...
def _fit_estimator(
    ...
    if model == "ea_fbcsp" and type(estimator).__module__.startswith("benchmark.research."):
...
def _feature_contract() -> dict[str, Any]:
    ...
    return {
        "schema": "eeg-mi-local-fixed-geometric-features-v1",
        "input": "eeg-mi-cache-v2 broadband trials after split-specific channel scaling",
        "sfreq_hz": sfreq,
