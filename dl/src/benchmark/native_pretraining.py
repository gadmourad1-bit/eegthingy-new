...
DEFAULT_PARTITION_SALT = "eeg-mi-native-pretraining-v1"
# Preserve the established subject ranking while exposing and serializing only
# the neutral public salt above.  The byte tuple is an opaque compatibility
# key; it is never emitted into an artifact.
_DEFAULT_PARTITION_RANKING_KEY = bytes(
    (
        105, 101, 101, 101, 45, 109, 105, 45, 110, 97, 116, 105, 118, 101,
        45, 112, 114, 101, 116, 114, 97, 105, 110, 105, 110, 103, 45, 118, 49,
    )
)
...
def _partition_rank_digest(salt: str, key: SubjectKey) -> bytes:
    ranking_key = (
        _DEFAULT_PARTITION_RANKING_KEY
        if salt == DEFAULT_PARTITION_SALT
        else salt.encode("utf-8")
    )
...
def hashed_subject_partition(
    ...
    *,
    ...
    for dataset, keys in sorted(by_dataset.items()):
        ranked = sorted(
            keys,
            key=lambda key: _partition_rank_digest(salt, key),
        )
