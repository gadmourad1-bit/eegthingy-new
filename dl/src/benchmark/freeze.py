...
FREEZE_MANIFEST_SCHEMA = "eeg-mi-confirmation-freeze-v1"
PREPROCESSING_PROFILE_SCHEMA = "eeg-mi-preprocessing-profile-v1"
...
def _source_entries(project_root: Path) -> dict[str, Any]:
    package_root = project_root / "src" / "benchmark"
    tests_root = project_root / "tests"
    if not package_root.is_dir():
        raise FileNotFoundError(
            f"project package directory does not exist: {package_root}"
        )
    python_paths = sorted(package_root.rglob("*.py"))
    if tests_root.is_dir():
        python_paths.extend(sorted(tests_root.rglob("*.py")))
        python_paths.sort()
    requirement_paths = sorted(package_root.rglob("requirements*.txt"))
    if not python_paths:
        raise ValueError(f"no project Python source files found below {package_root}")
    if not requirement_paths:
        raise ValueError(f"no project requirements file found below {package_root}")
...
def validate_freeze_manifest(
    ...
    *,
    ...
    if manifest.get("source_inventory") != current_inventory:
        raise ValueError(
            "current benchmark source/requirements bytes differ from the freeze"
        )
