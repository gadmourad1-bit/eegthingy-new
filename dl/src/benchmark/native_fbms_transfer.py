...
from .legacy_native_identity import (
    PINNED_LEGACY_SOURCE_INITIAL_STATE_SHA256,
    logical_class_name as _legacy_logical_class,
...
ARTIFACT_SCHEMA = "eeg-mi-native-cardinal-fbms-transfer-development-v1"
PREDICTION_SCHEMA = "eeg-mi-native-cardinal-fbms-transfer-predictions-v1"
...
def _validate_checkpoint_model_record(
    ...
    expected_class = _legacy_logical_class(type(reconstructed))
...
def _validate_checkpoint_source_provenance(
    ...
    validate_checkpoint_manifest(files, sorted(required))
...
def _frozen_partition_record() -> dict[str, object]:
    ...
    for dataset, keys in sorted(by_dataset.items()):
        ranked = sorted(
            keys,
            key=lambda key: _partition_rank_digest(config.partition_salt, key),
        )
...
def _validate_frozen_pretraining_contract(
    ...
    if initial_hash != PINNED_LEGACY_SOURCE_INITIAL_STATE_SHA256:
        raise RuntimeError(
            "CardinalFBMS source initial state differs from the pinned legacy writer identity"
        )
...
def _source_paths() -> dict[str, Path]:
    ...
    additions = {
        "eeg_mi/native_fbms_pretraining_cli.py": package
        / "native_fbms_pretraining_cli.py",
        "eeg_mi/native_fbms_transfer.py": Path(__file__).resolve(),
    }
...
def run_record(
    *,
    ...
    raise RuntimeError(
        "the historical native-FBMS v1 writer is retired after namespace "
        "migration; its archived source is validation evidence only"
    )
