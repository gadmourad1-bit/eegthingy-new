...
def _official_tcformer_module() -> type[nn.Module]:
    ...
    cached = sys.modules.get("_eeg_tcformer.models.tcformer")
    if cached is not None:
        if getattr(cached.TCFormerModule, "_eeg_provenance", None) != provenance:
            raise RuntimeError(
    ...
    package = types.ModuleType("_eeg_tcformer")
    package.__path__ = [str(root)]  # type: ignore[attr-defined]
    models_package = types.ModuleType("_eeg_tcformer.models")
    ...
    classification = types.ModuleType("_eeg_tcformer.models.classification_module")
    ...
    module.TCFormerModule._eeg_provenance = provenance  # type: ignore[attr-defined]
