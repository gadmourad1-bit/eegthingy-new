...
reconstruct a historical configuration. `benchmark.procedure_grid inventory`
produces the matching machine-readable view without opening any file below a
`results/` directory.

The `deepnet/...` strings below are predecessor-workspace path records copied
from the historical artifacts. They are not directories in the current source
tree and are not valid current commands.
...
- Stable-ID evidence:
  `benchmark.model_registry._ORBIT_CONFIGURATIONS` identifies v2 as the
...
- Stable-ID evidence:
  `benchmark.model_registry._ORBIT_CONFIGURATIONS` identifies v4 as the v3
...
- Recorded command: `-c`
- Stable-ID evidence:
  `benchmark.model_registry._ORBIT_CONFIGURATIONS` identifies v5 as the v4
