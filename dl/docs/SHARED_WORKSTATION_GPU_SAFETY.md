...
The retained `.eeg-mi-project-*` directory names are stable coordination
schema identifiers. They are not active Python package or distribution names.

Historical hash tables use `pre-migration-package/` as a token-free display
alias for the predecessor namespace. Those aliases are not current paths, and
their hashes identify the recorded predecessor bytes.
...
project-wide registry implemented by `src/benchmark/project_gpu_leases.py`.
...
The table records the literal pre-migration paths attached to that lab test;
it is historical evidence, not a current import or test command. The live
files are under `src/benchmark/` and have their current identities recorded by the
