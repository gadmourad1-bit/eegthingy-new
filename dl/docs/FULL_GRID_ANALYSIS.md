...
> **Reissued protocol identity.** Commands below use the active
> `benchmark`
> namespace. The distributed common-grid artifacts are a sanitized metadata
> reissue of the completed run: scientific tables and JSON numeric values are
> preserved, while plan, source-map, schema, and checksum identities are
> reissued. Predecessor digests provide lineage only and are not the current
> execution identity. Use `scripts/reproduce.sh reviewer` for bounded replay.

`benchmark.full_grid` and `benchmark.full_grid_analysis` define the reissued formal
...
python -m benchmark.full_grid preflight \
...
CUBLAS_WORKSPACE_CONFIG=:4096:8 \
uv run --frozen --no-default-groups --no-sync \
python -m benchmark.full_grid run \
  --run-root /absolute/path/to/common-grid-run \
  --cache-root /absolute/path/to/data_cache \
  --gpus 0,1,2 \
  --cpu-threads 4
...
```bash
uv run --frozen --no-default-groups --no-sync \
python -m benchmark.full_grid status \
  --run-root /absolute/path/to/common-grid-run

uv run --frozen --no-default-groups --no-sync \
python -m benchmark.full_grid audit \
  --run-root /absolute/path/to/common-grid-run \
...
- All formal experiment tracks share the verified project-root registry
  `.eeg-mi-project-gpu-leases`. It permits one worker per physical UUID and
...
CUBLAS_WORKSPACE_CONFIG=:4096:8 \
uv run --frozen --no-default-groups --no-sync \
python -m benchmark.full_grid_analysis \
  --run-root /absolute/path/to/common-grid-run \
  --cache-root /absolute/path/to/data_cache \
