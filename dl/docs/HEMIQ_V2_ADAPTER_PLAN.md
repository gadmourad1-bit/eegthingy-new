...
`predecessor-workspace/hemi_q/` are read-only provenance. A harmonized-v2 run is a
...
The source is the immutable `eeg-mi-cache-v2/harmonized` BNCI2014-004 cache:
...
`predecessor-workspace/hemi_q/hemi_q_final_frozen_manifest.json`. Only the input
...
`fa30acf45271e64eb9f9497a44aeb3ffd6d165beace733f55d5c950c0770ba65`.
It cites the historical parent manifest by path, 6,688-byte size, and SHA-256,
but `benchmark.hemiq_v2_model.load_frozen_config` does not open that
...
`benchmark.hemiq_v2_grid`. It accepts only the explicit 45-job roster above.
`benchmark.hemiq_v2_analysis` is an aggregate-only consumer: it obtains the
