...
> **Historical namespace record.** `pre-migration-package/` is a token-free
> display alias for the predecessor namespace used by the files and result
> locations verified at the recorded time. It is not a literal current path;
> the live package is `benchmark`, and the recorded hashes identify predecessor
> bytes.
...
`pre-migration-package/verification/verify_native_fbms_full_grid.py`, has SHA-256
...
`pre-migration-package/results/native_fbms_full_grid_independent_raw_verification.json`.
