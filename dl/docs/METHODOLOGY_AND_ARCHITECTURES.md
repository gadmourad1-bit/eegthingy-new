...
`benchmark.full_grid.COMMON_ARCHITECTURES` plus
`benchmark.baselines.make_model`. The broader ownership/provenance crosswalk is
`docs/INHOUSE_MODEL_METHODS.md` and the 70-entry registry is
`src/benchmark/model_registry.py`.

The token `eeg-mi-cache-v2` below is an immutable cache-schema identifier
from the completed protocol. It is not the active package or distribution
name; those are `benchmark` and `benchmark`.
...
Public-dataset trials use the `eeg-mi-cache-v2` harmonized profile: 4–40 Hz,
