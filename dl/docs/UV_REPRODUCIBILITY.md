# UV reproducibility contract for the EEG MI benchmark

## Status and scope

This document defines the dependency and environment contract for the clean
release root deployed on the lab workstation. It also retains the historical
audit record from the broader source repository. The
...
> **Namespace boundary.** The live package and distribution are
> `benchmark` and
> `benchmark`. `pre-migration-package/` is a token-free display alias
> used for predecessor path records; it is not a literal filesystem path.
> Recorded hashes still identify the predecessor bytes, not the reissued file
> at the analogous active path.
...
| `pre-migration-package/requirements-cu128.txt` | 257 | `06171d32aaef187b2ba9047322a8e4a93e5f16d04b743cb75095beda9f858a16` | Historical path of the legacy RTX 5070/cu128 environment, now retained at `src/benchmark/requirements-cu128.txt`; incompatible with the lab contract |
...
The in-repository code under the single `benchmark` namespace imports
...
| legacy `src/benchmark/research/*benchmark.py` procedures | CPU and GPU depending on model | Use the same scientific libraries, but predate the final UV lock contract |
...
- TCFormer at pinned commit
  ...
  `benchmark.tcformer_source`; Git metadata is not a runtime requirement.
...
uv run --frozen --no-default-groups --group test \
  python -m pytest -q --import-mode=importlib tests/core tests/research
```
...
uv run --frozen --no-default-groups --group test python -m pytest -q --import-mode=importlib \
  tests/core/test_development_screen.py \
  tests/core/test_conditioned_screen.py \
  tests/core/test_conditioned_analysis.py \
  tests/core/test_robustness_screen.py \
  tests/core/test_robustness_analysis.py \
  tests/core/test_full_grid.py \
  tests/core/test_full_grid_analysis.py \
  tests/core/test_control_grid.py \
  tests/core/test_chsd.py \
  tests/core/test_chsd_conditioned.py
```
...
uv run --frozen --no-default-groups \
  python -m benchmark.full_grid status --run-root <run-root>

uv run --frozen --no-default-groups python -m benchmark.control_grid audit \
