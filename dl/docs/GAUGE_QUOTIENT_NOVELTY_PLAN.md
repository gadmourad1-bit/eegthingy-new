...
| [GCNs-Net, TNNLS](https://doi.org/10.1109/TNNLS.2022.3202569) | graph filtering for MI | General graph overlap. Complete-graph edge differences are an algebraic interpretation here, not evidence of graph-method novelty. |
...
1. a systematic search across publisher indexes, PubMed, Scopus/Web of Science,
...
`src/benchmark/gauge_quotient_screen.py` is a read-only specification helper. It
...
`EEG_MI_UV_EXECUTABLE` to canonical absolute paths, then invokes that exact UV
