...
The retained `eeg-mi-curated-benchmark-summary-v1` token is an immutable
serialized-schema identifier. It is not the active package or distribution
name.
...
decimal numbers generated from finite binary64 values with Python's
shortest round-trip representation.

`results/benchmark_summary.json` contains the same rows, in the same order,
under schema `eeg-mi-curated-benchmark-summary-v1`. It additionally records
