...
EXPECTED_RESULTS_INVENTORY_SHA256 = (
    "846d3e1772d435eff3546aead006efedaa62b1fd5cfcd655b8db1dd2e67c7c1f"
)
TCFORMER = "tcformer"
EXTERNAL_COMMON_MODELS = frozenset(
    {
        "eegnet",
        "shallow",
        "deep4",
...
def registry_public_common_models() -> frozenset[str]:
    """Read the metadata-only registry's exact public common roster.

    The report builder remains isolated from the training package and its
    imports.  Parsing the registry assignment still makes the provenance-color
    contract fail closed if that canonical roster changes.
    """

    path = require_file(ROOT / "src" / "benchmark" / "model_registry.py")
    try:
        tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
    except (OSError, SyntaxError) as error:
        raise ReportInputError(f"cannot parse canonical model registry: {error}") from error
    ...
    for node in tree.body:
        if (
            ...
            and node.target.id == "_COMMON_PUBLIC"
        ):
            assignment = node.value
            break
    if not isinstance(assignment, (ast.Tuple, ast.List)):
        raise ReportInputError("canonical model registry lacks _COMMON_PUBLIC tuple")
    ...
    for entry in assignment.elts:
        if (
            ...
            or entry.func.id != "_common_public"
            or not entry.args
            or not isinstance(entry.args[0], ast.Constant)
            or not isinstance(entry.args[0].value, str)
        ):
            raise ReportInputError("canonical _COMMON_PUBLIC entry has an unexpected form")
        models.append(entry.args[0].value)
    if len(models) != len(set(models)):
        raise ReportInputError("canonical public common-model roster contains duplicates")
...
def validate_analysis_manifest() -> dict[str, Any]:
    ...
    if manifest.get("schema") != "eeg-mi-common-grid-analysis-manifest-v4":
...
def validate_inputs() -> ValidatedInputs:
    ...
    if plan.get("schema") != "eeg-mi-score-blind-common-grid-plan-v5":
    ...
    if audit.get("schema") != "eeg-mi-score-blind-common-grid-audit-v3":
    ...
    if (
        preflight.get("schema") != "eeg-mi-common-track-cuda-preflight-v3"
    ...
    if (
        receipt.get("schema") != "eeg-mi-common-track-cuda-preflight-receipt-v1"
    ...
    for index, row in enumerate(ranking, 1):
        ...
        if wide["status"] != "descriptive_opened_development_suite_only":
            raise ReportInputError("balanced-accuracy view has an unexpected evidence status")
    if any(
        float(ranking[index]["value"]) > float(ranking[index - 1]["value"])
        for index in range(1, EXPECTED_MODELS)
    ):
        raise ReportInputError("balanced-accuracy ranking is not nonincreasing")

    model_set = set(models)
    registry_public_models = registry_public_common_models()
    if (
        len(EXTERNAL_COMMON_MODELS) != 15
        or EXTERNAL_COMMON_MODELS != registry_public_models
        or not EXTERNAL_COMMON_MODELS < model_set
        or len(model_set - EXTERNAL_COMMON_MODELS) != 28
    ):
        raise ReportInputError(
            "external common-model roster must match the registry's exact 15 public configurations"
        )
    ...
    accuracy_ranks = tuple(
        exact_int(row["descriptive_accuracy_rank"], label="accuracy rank")
        for row in accuracy_wide
    )
    if accuracy_ranks != tuple(range(1, EXPECTED_MODELS + 1)):
        raise ReportInputError("accuracy view is not ordered by consecutive ranks 1..43")
    balanced_rank_by_model = {
        row["model"]: exact_int(row["rank"], label="balanced rank")
        for row in balanced_wide
    }
    for row in accuracy_wide:
        model = row["model"]
        if exact_int(
            ...
            label="accuracy-view primary balanced rank",
        ) != balanced_rank_by_model[model]:
            raise ReportInputError(
                f"accuracy view has the wrong primary balanced rank for {model}"
            )
        if row["status"] != "descriptive_opened_development_suite_only":
            raise ReportInputError("accuracy view has an unexpected evidence status")
        assert_close(
    ...
    if any(
        float(accuracy_wide[index]["equal_dataset_macro_accuracy"])
        > float(accuracy_wide[index - 1]["equal_dataset_macro_accuracy"])
        for index in range(1, EXPECTED_MODELS)
    ):
        raise ReportInputError("standard-accuracy ranking is not nonincreasing")
    ...
    if (
        gauge.get("schema") != "eeg-mi-gauge-gate1-decision-v1"
    ...
    if (
        ...
        != "eeg-mi-cardinal-fbms-post-outcome-independent-verification-v1"
...
def all_models_chart(
    ...
    *,
    ...
    """Draw one complete, absolute-scale ranking of all common-grid models."""

    if metric == "balanced_accuracy":
        rows = [
            (
                exact_int(row["rank"], label="all-model BA chart rank"),
                row["model"],
                finite_float(
                    row["equal_dataset_macro_balanced_accuracy"],
                    label="all-model BA chart value",
                    minimum=0.0,
                    maximum=1.0,
                ),
            )
        ...
        axis_label = "Equal-dataset macro balanced accuracy"
    elif metric == "accuracy":
        rows = sorted(
            (
                exact_int(
                    row["descriptive_accuracy_rank"],
                    label="all-model accuracy chart rank",
                ),
                row["model"],
                finite_float(
                    row["equal_dataset_macro_accuracy"],
                    label="all-model accuracy chart value",
                    minimum=0.0,
                    maximum=1.0,
                ),
            )
        ...
        axis_label = "Equal-dataset macro standard accuracy"
    else:
        raise ValueError(f"unsupported all-model chart metric: {metric}")

    if len(rows) != EXPECTED_MODELS or tuple(rank for rank, _, _ in rows) != tuple(
        range(1, EXPECTED_MODELS + 1)
    ):
        raise ReportInputError("all-model chart rows are not the exact ranked 43-model roster")
    if {model for _, model, _ in rows} != set(inputs.models):
        raise ReportInputError("all-model chart roster differs from the frozen plan")
    if max(value for _, _, value in rows) > 0.80:
        raise ReportInputError("all-model chart value exceeds its absolute 0%-80% axis")

    chart_leader = rows[0][1]
    ...
    row_height = chart_height / EXPECTED_MODELS
    x_min, x_max = 0.0, 0.80

    legend = (
        ("Project configuration", BLUE),
        ("External common configuration", GOLD),
        ("Observed leader", TEAL),
    )
    legend_x = 0.0
    ...
    for tick in (0.0, 0.20, 0.40, 0.60, 0.80):
        x = left + (tick - x_min) / (x_max - x_min) * chart_width
        drawing.add(
            Line(
                x,
    ...
    for index, (rank, model, value) in enumerate(rows):
        y = top - (index + 0.72) * row_height
        label_color = NAVY if model == chart_leader else INK
        drawing.add(
            String(
                left - 6,
                y,
                f"{rank:>2}. {ascii_text(model)}",
                fontName="Helvetica-Bold" if model == chart_leader else "Helvetica",
                fontSize=6.15,
                textAnchor="end",
                fillColor=label_color,
            )
        )
        if model == chart_leader:
            bar_color = TEAL
        elif model in EXTERNAL_COMMON_MODELS:
            bar_color = GOLD
        else:
            bar_color = BLUE
        bar_width = (value - x_min) / (x_max - x_min) * chart_width
        drawing.add(
            Rect(
                left,
                y - 1.0,
                bar_width,
                max(3.7, row_height * 0.56),
                strokeColor=None,
                fillColor=bar_color,
            )
        )
    ...
    drawing.add(
        String(
            left + chart_width / 2,
            1,
            f"{axis_label} (absolute 0%-80% scale)",
            fontName="Helvetica",
            fontSize=6.3,
            textAnchor="middle",
            fillColor=MUTED,
...
def build_results_story(inputs: ValidatedInputs, doc: ReportDocTemplate) -> list[Any]:
    ...
    story.extend(
        [
            PageBreak(),
            heading("All 43 models: balanced accuracy", 1, styles),
            Paragraph(
                "Complete primary ranking by equal-dataset macro balanced accuracy. "
                "The axis begins at zero; exact values are printed. Gold bars identify "
                "the 15 external common configurations, which are shared-recipe "
                "architecture comparisons rather than exact original-paper recipe "
                "reproductions. Colors indicate provenance, not statistical significance.",
                styles["Small"],
            ),
            Spacer(1, 3),
            all_models_chart(
            ...
            PageBreak(),
            heading("All 43 models: standard accuracy", 1, styles),
            Paragraph(
                "Complete descriptive secondary ranking by equal-dataset macro standard "
                "accuracy. This ordering is computed independently of the primary "
                "balanced-accuracy rank. The axis begins at zero and exact values are "
                "printed; colors indicate implementation provenance only.",
                styles["Small"],
            ),
            Spacer(1, 3),
            all_models_chart(
