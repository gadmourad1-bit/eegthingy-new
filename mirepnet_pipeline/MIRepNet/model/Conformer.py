...
class PatchEmbedding(nn.Module):
    def __init__(self, emb_size=40,num_cha=22):
        super().__init__()
        self.shallownet = nn.Sequential(
            nn.Conv2d(1, 40, (1, 25), (1, 1)),
            nn.Conv2d(40, 40, (num_cha, 1), (1, 1)),
            nn.BatchNorm2d(40),
            nn.ELU(),
...
class Conformer(nn.Module):
    def __init__(self, emb_size=40, depth=6, n_classes=2,num_cha=22, **kwargs):
        ...
        #self.clshead = ClassificationHead(n_classes=n_classes)
        self.clshead = nn.Linear(emb_size,n_classes)
    def forward(self, x):
        #x=x.squeeze(-1)
        # print("x = self.patch_embedding(x)")
        x = self.patch_embedding(x)  # [batch_size, seq_length, emb_size]
