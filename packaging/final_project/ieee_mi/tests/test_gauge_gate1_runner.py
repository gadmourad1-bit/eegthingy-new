...
from ieee_mi import gauge_gate1_analysis as analysis
from ieee_mi import gauge_gate1_runner as gate
...
def _allow_darwin_synthetic_rename(
    ...
    if sys.platform == "darwin":
        monkeypatch.setenv("IEEE_MI_DARWIN_SYNTHETIC_TEST", "1")
...
def _fake_receipt(plan: dict[str, Any], gpu: str = DUMMY_GPUS[0]) -> dict[str, Any]:
    ...
    return {
        "schema": "ieee-mi-project-gpu-lease-receipt-v1",
        "lease": {
            "schema": "ieee-mi-project-gpu-lease-v1",
...
def test_plan_source_closure_includes_novelty_and_analyzer(
    ...
    assert "ieee_mi/gauge_gate1_analysis.py" in plan["source_identity"]
