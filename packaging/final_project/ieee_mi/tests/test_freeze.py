...
from ieee_mi.config import dataset_spec
from ieee_mi.freeze import (
...
def test_freeze_manifest_hashes_complete_source_checkpoint_and_protocol(
    ...
    assert profile["preprocessing"]["schema"] == "ieee-mi-cache-v2"
    assert profile["channel_scaling"]["schema"] == "ieee-mi-channel-scaling-v1"
...
def test_freeze_validation_detects_every_referenced_byte_change(
    ...
    if mutation == "source":
        (project / "ieee_mi" / "model.py").write_text(
            "MODEL = 'changed'\n",
            encoding="utf-8",
        )
        message = "source/requirements bytes differ"
    elif mutation == "added_source":
        (project / "ieee_mi" / "new_source.py").write_text(
            "NEW = True\n",
            encoding="utf-8",
        )
        message = "source/requirements bytes differ"
