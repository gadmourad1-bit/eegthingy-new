...
`ieee_mi.control_grid_analysis` is the fail-closed analysis companion to the
...
uv run --frozen --no-sync python -m ieee_mi.control_grid_analysis \
...
python -m pytest -q ieee_mi/tests/test_control_grid_analysis.py
```
