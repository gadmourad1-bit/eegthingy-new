...
`deepnet.local_outer_refit_benchmark` uses four immutable, configuration-only
JSON files:
...
No path below `deepnet/results/` is part of the runtime source closure.
...
python -m pytest -q deepnet/tests/test_local_outer_refit_benchmark.py
```
