...
[`ieee_mi/model_registry.py`](../ieee_mi/model_registry.py). It records what
...
[`ieee_mi/models.py`](../ieee_mi/models.py), with construction in
[`ieee_mi/baselines.py`](../ieee_mi/baselines.py). The project repository
...
| EEGConformer | `eegconformer`, `eegconformer_compact` | [Song et al.](https://ieeexplore.ieee.org/document/9991178); six Transformer layers versus a local four-layer compact configuration. |
...
Existing architecture/deployment results are in `deepnet/results` and are
summarized in [`MODEL_ACCURACY_INVENTORY.md`](../MODEL_ACCURACY_INVENTORY.md).
...
[`ieee_mi/reference_training.py`](../ieee_mi/reference_training.py):
...
FBMSNet reproduction. Complete full-grid statistics are in
[`ieee_mi/results/NATIVE_FBMS_FULL_GRID_SUMMARY.md`](../ieee_mi/results/NATIVE_FBMS_FULL_GRID_SUMMARY.md).
...
The authoritative existing-result inventory is
[`MODEL_ACCURACY_INVENTORY.md`](../MODEL_ACCURACY_INVENTORY.md). The exact
current local 50-condition audit is
...
[`ieee_mi/tests/test_model_registry.py`](../ieee_mi/tests/test_model_registry.py)
checks:
