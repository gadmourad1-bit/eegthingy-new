...
if [[ -n "${IEEE_MI_TCFORMER_ROOT:-}" \
      && "$IEEE_MI_TCFORMER_ROOT" != "$EXPECTED_TCFORMER_ROOT" ]]; then
    fail "IEEE_MI_TCFORMER_ROOT must use the vendored release snapshot."
fi
export IEEE_MI_TCFORMER_ROOT="$EXPECTED_TCFORMER_ROOT"
