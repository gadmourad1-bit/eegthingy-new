...
This runner is intentionally not part of :mod:`ieee_mi.full_grid`.  It applies
the released optimization recipes implemented in
:mod:`ieee_mi.reference_training` to the same five opened v2 caches and split
identities, then publishes only row identities and probabilities.  Scores are
joined to labels later by :mod:`ieee_mi.author_recipe_analysis`.
...
PLAN_SCHEMA = "ieee-mi-author-recipe-grid-plan-v1"
RECORD_SCHEMA = "ieee-mi-author-recipe-score-blind-record-v1"
COMPLETION_SCHEMA = "ieee-mi-author-recipe-completion-v1"
CLAIM_SCHEMA = "ieee-mi-author-recipe-claim-v1"
FAILURE_SCHEMA = "ieee-mi-author-recipe-failure-v1"
AUDIT_SCHEMA = "ieee-mi-author-recipe-audit-v1"
ANALYSIS_CONTRACT_SCHEMA = "ieee-mi-author-recipe-analysis-contract-v1"
PUBLICATION_FENCE_SCHEMA = "ieee-mi-author-recipe-publication-fence-v1"
...
def _tcformer_source_root() -> Path:
    value = os.environ.get("IEEE_MI_TCFORMER_ROOT")
    if not value:
        raise AuthorRecipeError(
            "IEEE_MI_TCFORMER_ROOT must name the pinned vendored source"
        )
...
def _recipe_contracts() -> dict[str, Any]:
    ...
    return {
        "reference.tcformer": {
            ...
            "factory_callable": "ieee_mi.baselines.make_model(tcformer)",
        ...
        "reference.fbcnet": {
            ...
            "factory_callable": (
                "PrefilteredFBCNetAdapter(ieee_mi.baselines.make_model(fbcnet))"
            ),
            "config": fbcnet,
...
def _analysis_contract(
    ...
    project_root = Path(__file__).resolve().parents[1]
    analysis_path = project_root / "ieee_mi" / "author_recipe_analysis.py"
    ...
    return {
        ...
        "source_identity": {
            "ieee_mi/author_recipe_analysis.py": _sha256_file(analysis_path),
        },
...
def _require_uv_virtual_environment() -> None:
    ...
    _validate_uv_project_contract(Path(__file__).resolve().parents[1])
