...
MATRIX_SCHEMA: Final[str] = "ieee-mi-track-execution-matrix-v2"
...
_DESCRIPTORS: list[ExecutionDescriptor] = [
    ...
    _descriptor(
        ...
        "deepnet.filterbank_net.FilterBankSPDClassifier.fit",
        "geoadapt_fb",
        "broadband raw EEG",
        _BINARY_CONSTRAINT,
...
for stable_id, model_key, component_callable, identity in (
    (
        "architecture.cameo",
        "cameo",
        "deepnet.cameo_net.CAMEOClassifier.fit",
        "Frozen CAMEOConfig and its declared mixture/rho candidate set.",
    ),
    (
        "architecture.hemiparity",
        "hemiparity",
        "deepnet.parity_net.HemiParityClassifier.fit",
        "Frozen ParityConfig.",
    ),
    (
        "architecture.parity_fuse",
        "parity_fuse",
        "deepnet.parity_fuse_net.ParityFuseClassifier.fit",
...
_DESCRIPTORS.extend(
    [
        _descriptor(
            ...
            "deepnet.hemi_q_field_net.HemiQFieldClassifier.fit",
            "hemi_q_field",
...
for version in ("v1", "v2", "v3", "v4", "v5"):
    _DESCRIPTORS.append(
        _descriptor(
            ...
            "deepnet.orbit_transport_net.OrbitTransportClassifier.fit",
            f"orbit_{version}",
...
_DESCRIPTORS.extend(
    [
        _descriptor(
            "reference.tcformer",
            "ieee_mi.reference_training.fit_tcformer_reference",
            "tcformer",
        ...
        _descriptor(
            "reference.fbcnet",
            "ieee_mi.reference_training.fit_fbcnet_reference",
            "fbcnet",
...
for stable_id, condition in (
    ...
    _DESCRIPTORS.append(
        _descriptor(
            stable_id,
            "ieee_mi.native_transfer.run_record",
            condition,
            ...
            grid_kind="native_target",
            configuration_identity=f"ieee_mi.native_transfer condition={condition!r}.",
        )
    )
...
for stable_id, condition in (
    ...
    _DESCRIPTORS.append(
        _descriptor(
            stable_id,
            "ieee_mi.native_fbms_transfer.run_record",
            condition,
            ...
            grid_kind="native_target",
            configuration_identity=f"ieee_mi.native_fbms_transfer condition={condition!r}.",
        )
    )
...
for stable_id, component_callable, runtime_key, representation in (
    ...
    (
        ...
        "deepnet.tangent_anchor.TangentAnchorClassifier.fit",
        "tangent_anchor",
        "four-band SPD covariance tensors",
    ),
    (
    ...
    _DESCRIPTORS.append(
        _descriptor(
            ...
            "ieee_mi.local_geometric_controls.fixed_filter_bank; use "
            ...
            configuration_identity=(
                "Frozen hyperparameter grids in ieee_mi.local_geometric_controls."
            ),
        )
    )
...
_ready_adapters: dict[tuple[str, str], str] = {
    ("architecture.cameo", "local_exp4"): (
        "deepnet.local_outer_refit_benchmark.run_benchmark"
    ),
    ...
    ("architecture.hemi_q_field", "bnci2014_004"): (
        "ieee_mi.hemiq_v2_grid.execute_job"
    ),
}
            ...
            if record.track == "common":
                ...
                cells.append(
                    TrackCell(
                        ...
                        adapter_callable="ieee_mi.full_grid.execute_benchmark_job",
...
def validate_track_matrix() -> None:
    ...
    if not _source_symbol_exists("ieee_mi.full_grid.execute_benchmark_job"):
    ...
    full_grid_constants = _source_constants("ieee_mi/full_grid.py")
    ...
    native_constants = _source_constants("ieee_mi/native_transfer.py")
    ...
    legacy_conditions = {
        ...
        for descriptor in EXECUTION_DESCRIPTORS
        if descriptor.component_callable == "ieee_mi.native_transfer.run_record"
    }
    ...
    fbms_constants = _source_constants("ieee_mi/native_fbms_transfer.py")
    fbms_conditions = {
        descriptor.runtime_key
        for descriptor in EXECUTION_DESCRIPTORS
    ...
    control_constants = _source_constants("ieee_mi/control_grid.py")
