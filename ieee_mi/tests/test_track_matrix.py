...
from ieee_mi.model_registry import BENCHMARK_DATASETS, MODEL_REGISTRY
from ieee_mi.track_matrix import (
    COMMON_FULL_GRID,
    DATASET_GRIDS,
    DESCRIPTOR_BY_ID,
    EXECUTION_BACKLOG,
...
def test_ready_paths_are_outer_cache_bound_callables() -> None:
    for stable_id in (
        ...
        assert cell.adapter_callable != "deepnet.local_outer_refit_benchmark.run_one"
    for stable_id in (
        "control.riemann",
        "control.tangent_anchor",
        "control.ea_fbcsp",
        ...
        for dataset in BENCHMARK_DATASETS:
            ...
            assert cell.adapter_callable == "ieee_mi.control_grid.main"
            assert cell.seeds == ()
            assert cell.atomic_jobs == cell.result_records
    hemiq = _cell("architecture.hemi_q_field", "bnci2014_004")
    assert hemiq.adapter_callable == "ieee_mi.hemiq_v2_grid.execute_job"
...
def test_registry_identity_component_and_adapter_are_not_conflated() -> None:
    ...
    assert common.adapter_callable == "ieee_mi.full_grid.execute_benchmark_job"
    ...
    assert local_procedure.component_callable == "deepnet.cameo_net.CAMEOClassifier.fit"
    assert (
    ...
    assert control.adapter_callable == "ieee_mi.control_grid.main"
...
def test_duplicate_callables_are_explicitly_visible() -> None:
    ...
    assert groups["deepnet.filterbank_net.FilterBankSPDClassifier.fit"] == (
        "architecture.geoadaptnet_fb",
        "architecture.geoadaptnet_fbsp",
    )
    assert groups["deepnet.orbit_transport_net.OrbitTransportClassifier.fit"] == (
    ...
    assert len(groups["ieee_mi.native_transfer.run_record"]) == 4
    assert len(groups["ieee_mi.native_fbms_transfer.run_record"]) == 5
    ...
    assert manifest["proposed_cardinality_totals"] == {
        ...
        "by_status": {
            COMMON_FULL_GRID: {
                ...
                MISSING_ADAPTER: {
                    "atomic_execution_units": 30_110,
                    "result_records": 30_110,
            },
            REGISTRY_NA: {
                ...
                SEPARATE_ELIGIBLE: {
                    "atomic_execution_units": 17_164,
                    "result_records": 17_164,
            },
        },
    }
